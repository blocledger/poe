  ...
  describe('/verifyDoc', function() {
    ...
    it('should report an error for an unknown hash value', function(done) {
      ...
      .end(function(err, res) {
        // console.log(res);
        assert.isNotNull(err);
        assert.equal(res.status, 500, 'should recieve a 500 status');
...
describe('Blockchain utilities', function() {
  describe('updateChain()', function() {
    it('should update the chain height', function(done) {
      this.timeout(30000);
      ...
      request
      .post(url + '/addDoc')
      .send({hash: hash1, name: 'dummyFile.txt', owner: 'Alice', hashType: 'sha256'})
      .end(function(err, res) {
        assert.isNull(err);
        var newHeight = 0;
      ...
      it('then cleans up the test doc with deleteDoc', function(done) {
        this.timeout(6000);
        request
        .post(url + '/delDoc')
        .send({hash: hash1})
